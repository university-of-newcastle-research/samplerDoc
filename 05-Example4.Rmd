...
Here is our complete log likelihood function. You can copy this function if you'd like to follow on with our example
...
SDT_loglike <- function(x, data, sample = FALSE){
```
* `x` is a vector of parameter names
* `data` is the dataset
* `sample =` sample values (For this simple example, we do not require a `sample` argument.)

The first if statement (line 2) checks if you want to sample.......... and assigns NAs to your data frames response column. If you're not sampling (like us in this example), you need to create an output vector "`out`", that will contain the likelihood values for each row/trial and assign NAs to the out column of our data frame - why? <b> Need to explain the `(sample)` part below </b>
...
From line 16 (`}else{` i.e. `stim[i] == "non-word"`), we have the function for <i>non-word</i> trials. As can be seen below, the output value `(out[i])` for these trials is arrived at in a similar manner to the <i>word</i> trials. We set the `mean` to 0 and the standard deviation (`sd`) to 1. The criterion and upper/lower tails are the same as . This will give me values for the non-word distribution given the different responses.  
