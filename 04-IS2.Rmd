...
In this example we run IS2 to compare the four models in an unbiased manner. The [IS2 paper](https://link.springer.com/article/10.3758/s13428-020-01348-w) uses the same method, and so this chapter provides a walkthrough of how this example was run. As we are using samples from the PMwG algorithm, the prior_dist function is specifically coded for PMwG priors. To run other models, the prior_dist function needs to be updated (more on this later maybe). 
...
library(condMVNorm)
library(parallel)
setwd("~/Documents/Research/Modelling Project/Work/IS2/Tutorial")
...
IS_samples = 1000 #number of importance samples
