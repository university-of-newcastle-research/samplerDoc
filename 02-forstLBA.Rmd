...
```
 ...
 2.  Pass the data generating parameter values (`dgpv`) into the log-likelihood and generate data using your log-likelihood function, where `sample = TRUE`.
```{r testdatacheck2, include=TRUE}
test_data <- tw_lba_ll(x = dgpv,
                       data = data,
                       sample = TRUE
                       )
```

 3. Generate profile plots to compare the log-likelihood of the `dgpv` parameter values against nearby values in the 'synethetic' data. You can write your own function for this, or use the function below. 
...
incorrect_ll_plots <- test_ll(x = dgpv,
                              n_values = 9,
                              synth_data = test_data,
                              dims = c(2,2),
