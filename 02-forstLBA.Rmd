...
We demonstrate the application of the LBA with the PMwG sampler in a study of perceptual decision making. @forstmann2008striatum looked at neural correlates of decision making under time pressure, with an aim to identify areas of the brain associated with speed-accuracy tradeoff. Imaging (fMRI) and behavioural data was collected; however, we will analyse behavioural data from the decision-making task only. In terms of modelling the data, Forstmann expected to find differences in thresholds for each of the three speed-emphasis conditions. We have included the Forstmann et als data in the `PMwG` package as a data frame named `forstmann`. The sampler requires a data frame with a <b>`subject`</b> column. The subject column data type can be a factor or numeric.
...
```
 ...
 2.  Pass the data generating parameter values (`x`) into the log-likelihood and generate data using your log-likelihood function, where `sample = TRUE`.
```{r testdatacheck2, include=TRUE, warning=FALSE}
test_data <- fast_lba_ll3b(x = x,
                       data = data,
                       sample = TRUE
                       )
```

 3. Generate profile plots to compare the log-likelihood of the `x` parameter values against nearby values in the 'synethetic' data. You can write your own function for this, or use the function below. 

Our plotting function takes five arguments: 

* `x`  is a named parameter vector
* `n_values` number of parameter x-values we will assess to compare to the data-generating parameter values. Default is 9.
* `synth_data` your synthetic/simulated data
* `dims` plot dimensions. Here we have 7 parameters, so we display 2 plots per dimension.
* `ll_func`  name of log-likelihood function to be used
...
incorrect_ll_plots <- test_ll(x = x,
                              n_values = 9,
                              synth_data = test_data,
                              dims = c(2,2),
