...
The log-likelihood function is the 'engine' of the PMwG sampler. It takes in data, parameter values and estimates how likely those parameter values are given the associated data. This process is completed for each data point or trial and the estimates for each data point/trial are summed to obtain a likelihood value for those input parameter values. For example, we could compute the likelihood of a response time given a set of random effect values for a model. If the random effect values are more likely (i.e. the model is closer to the data) then the likelihood values will be higher for each data point. 
 
In the likelihood function, the parameter values must be assigned to the correct conditions and model parameters. For example, a linear regression model could have a slope and intercept parameter for easy and hard conditions i.e. four parameters. In this case, we would need to map the input parameter value for the slope in the easy conditon onto the easy condition's slope model parameter and then do the same for the hard slope and intercept parameters. We would then use these parameter values to work out how likely the data were given these values (for each condition). Then we can calculate how likely the parameter values are for their associated data points for each subject. 
