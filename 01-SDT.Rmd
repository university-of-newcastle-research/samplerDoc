...
### Writing a log-likelihood function{#SDTLLFun}

The log-likelihood function is the 'engine' of the PMwG sampler. It takes in data, parameter values and estimates how likely those parameter values are given those data. This process is completed for each data point or trial and the estimates for each data point/trial are summed to obtain a likelihood value for the model. The likelihood value is only meaningful when compared to another model's likelihood. For example, model A's likelihood may be -1000 and model B's likelihood may be -2500. In the case, model A's likelihood is greater than model B's likelihood, therefore, given the data, model A's parameter values are more likely than the model B's parameter values i.e. we seek the maximum likelihood.

In the likelihood function, the parameter values must be assigned to the correct conditions and model parameters. For example, a linear regression model could have a slope and intercept parameter for easy and hard conditions i.e. four parameters. In this case, we would need to map the parameter value for the slope in the easy conditon onto the easy condition's slope model parameter and then do the same for the hard slope and intercept parameters. We would then use these parameter values to work out how likely the data were given these values (for each condition). Then we can calculate how likely the parameter values are for their associated data points for each subject. 
...
As you can see in \@ref(fig:parPlot), the traces are clearly stable. Note that the number of iterations for the adaptation stage here is quite short because it's easy to estimate and so gets lots of unique draws.
