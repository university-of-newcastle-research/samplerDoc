...
For more details about the experiment please see [the original paper](https://www.sciencedirect.com/science/article/pii/S0749596X07000496).  <b> Is this the correct paper to reference? I hve changed the name of the columns in the wagenmakers dataset to the names used in the table shown below.</b>
...
Line 1 through to line 8 are the same as the log likelihood we wrote for the fabricated dataset above, so we'll begin from line 9. 

<br>
We begin by calculating the log-likelihood for <i>word</i> condition trials  and where the stimulus is High Frequency (freq="hf") for each response. 

Again, notice below that we find the upper tail when looking at "word" responses (ie the probability that we are responding word) and lower tail when looking at "non-word" responses. We look at these probabilities given the criterion - which in this scenario is the criterion for the condition where there is a higher proportion of words. 

NOTE: we use log.p=TRUE to log scale these values to make for an easy summation after these have been calculated.
