...
x = the sampler object including parameters that was created from the init function above.(stage = the sampling stage (These are:"burn", "adapt" or "sample" - in this order).(iter = is the number of iterations for the sampling stage. This is similar to running deMCMC, where it takes many iterations to reach the posterior space. Default = 1000.(particles = is the number of particles generated on each iteration. Default = 1000.(epsilon = is a value between 0 and 1 which reduces the size of the sampling space. We use lower values of epsilon when there are more parameters to estimate. Default = 1.(n_cores = the number of cores on a machine you wish to use to run the sammpler. This allows sampling to be run across cores (parallelising for subjects). Default = 1.)
