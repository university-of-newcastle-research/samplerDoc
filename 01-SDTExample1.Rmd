...
tidy.data <- do.call(rbind, data)
tidy.pp.data <- do.call(rbind, pp.data)
```


And now we can plot samples against the data.
```{r part12graph, echo=FALSE}
ggplot(data = av.tmp, aes(x = cond, y = word_prop)) + geom_col(alpha = 0.3) +
  geom_point(data = av.pp, aes(x = cond, y = word_prop)) + theme_classic()
```

Shown above is 20 posterior draws (dots) plotted against the data (bars). The posterior draws are for each individual subject - shown here is the average proportion of word responses. Evidently, the model appears to fit the data well.
