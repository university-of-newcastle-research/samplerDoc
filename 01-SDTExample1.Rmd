...
```{r part4.4,attr.source='.numberLines startFrom="28"', eval=FALSE}
sum(out)
```
...
Now we'll write a log-likelihood function for Wagenmakers experimental data.

which can account for the different word frequencies and the two word proportion conditions. Further, i make this function so it is faster than the lin-by-line version. We strongly recommend doing a line-by-line log-likeihood to make sure the function is functioining correctly, before doing a faster version. 


In this example, there are 5 parameters: 3 for the d' given by frequency (hf, lf, vlf) and 2 for the criterion, to model differences between the word proportion conditions (75% words vs 75% non-words). 

These are specified as: HF.d, LF.d, VLF.d, C.w, C.nw.
...
VLF.d = Very low frequency d'

C.w = Criterion for word proportion

C.nw = Criterion for non-word proportion
