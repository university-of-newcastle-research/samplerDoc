...
# Wagenmakers dataset
pqWag <- wagenmakers %>% 
  group_by(condition, subject, stim) %>% 
  summarise(Q25 = quantile(rt, prob = 0.25),
            median = median(rt), 
            Q75 = quantile(rt, prob = 0.75),
            acc =  mean(ifelse(correct == 2, 1, 0)),
            .groups = "keep"
            )
...
levels(av_pqWag$condition) <-
  levels(pp_varWag$condition) <- c("75% Non-Word stimuli","75% Word stimuli")
