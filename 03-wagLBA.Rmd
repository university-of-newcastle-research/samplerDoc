...
Here is a faster log-likelihood function that only calls `dLBA` once when `sample = FALSE`.
...
Here we cover tests to see if there are any errors in internal consistency with our loglikelihood function. 
...
tw_lba_ll(x, data, sampl = FALSE)
fast_lba_ll(x, data, sample = FALSE)
```
