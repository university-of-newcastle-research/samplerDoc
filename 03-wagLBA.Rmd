...
A description of the Wagenmakers experiment and data is covered in [chapter 1](#sdtOutline). This experiment is more complicated than the Forstmann example in [chapter 3](#forstmannChapter), and the LBA is also more complicated than the SDT model. As a result, the log-likelihood function for this exampl will be more complex, however, you'll notice that every step we take closely follows those taken in previous chapters with simpler data sets and simpler models. 
...
The structure of our log-likelihood function follows those in the preceding chapters, so we will only step through the parts that differ e.g. the different experimental design and the different hypothesis we have about which parameters are being influenced by the experimental manipulations.
