...
tw_lba_ll <- function(x, data, sample = FALSE) {
  ...
  for (i in 1:nrow(data)) {
    ...
    bs = list(b.nw, b.w)
    v.w = x[paste0("v.", data$stimulus[i], ".W")]
    v.nw = x[paste0("v.", data$stimulus[i], ".NW")]
    vs = c(v.nw, v.w)
    t0 = x["t0"]
    s = c(1, 1)
...
fast_lba_ll <- function(x, data, sample = FALSE) {
  ...
  b.w = b.nw = v.w = v.nw = out = numeric(nrow(data))
    for (p in unique(data$cond)) {
      use <- data$cond == p
      b.w[use] = x[paste0("b.", p, ".W")] + x["A"]
      b.nw[use] = x[paste0("b.", p, ".NW")] + x["A"]
    }
    for (f in unique(data$stimulus)) {
      use <- data$stimulus == f
...
sampler <- pmwgs(data = wagenmakers,
                 pars = pars,
                 prior = priors,
                 ll_func = fast_lba_ll
                 )
