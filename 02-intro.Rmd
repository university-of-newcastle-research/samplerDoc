...
To generate posterior predictive data we set `sample = TRUE` in the log-likelihood function above, which generates rt and responses given the posterior parameter estimates. 

Below is code you can use to simulate posterior predictive data from the `sampled` object loaded in your R-studio environment. `n` specifies the number of posterior samples. To generate this you will to load your `sampled` object for the 3b model.

```{r genPPDataFunc, Include=TRUE}
gen_ppdata <- function (sampled, n, ll_func = sampled$ll_func, rbind.data = TRUE) {
...
pp_data_3b <- gen_ppdata(sampled, n = 20)
```
...
Now we will plot the posterior predictive data against the real data. You may choose to plot this in other ways such as the interquartile range or with other packages. 

This may only be useful for some, as what you do with your data is domain specific.
...
pq <- data %>% 
  ...
  summarise_at(vars(rt),
               list(Q1 = ~quantile(.,probs = 0.25),
                    median = median, 
                    Q3 = ~quantile(., probs = 0.75)))


# pq = data %>% group_by(condition, subject) %>% summarise_at(vars(rt), funs(qL, medianNA, qH))
tmp <- data %>%
  group_by(condition, subject) %>%
  summarise(acc = mean(resp == stim))
...
post_varsrc <- factor(post_var$src,
  levels = c("model", "data"),
  labels = c("model", "(Data)")
)

## Change some level labels, for pretty plotting.
av_pq$condition <- factor(av_pq$condition, levels = c("1", "2", "3"))
post_var$condition <- factor(post_var$condition, levels = c("1", "2", "3"))

levels(av_pq$condition) <-
  levels(post_var$condition) <- c("Accuracy", "Neutral", "Speed")

## Make for msec and percentages.
av_pq$acc = 100 * av_pq$acc
post_var$acc = 100 * post_var$acc
av_pq[, c("Q1", "median", "Q3")] <- 1000 * av_pq[, c("Q1", "median", "Q3")]
post_var[, c("Q1", "median", "Q3")] <- 1000 * post_var[, c("Q1", "median", "Q3")]
  ...
  rtplot = rtplot + labs(x = "Emphasis Condition", y = "RT Percentiles (ms.)", colour =
                           "Model")
  
  
  rtplot = rtplot + geom_point(
    data = post_var,
    aes(x = condition, y = Q1, group = src),
    size = 1,
    color = "black",
    alpha = 0.2
  )
  rtplot = rtplot + geom_point(
    data = post_var,
    aes(x = condition, y = Q3, group = src),
    size = 1,
    color = "black",
    alpha = 0.2
  )
  rtplot = rtplot + geom_point(
    data = post_var,
    aes(x = condition, y = median, group = src),
    size = 2,
    color = "black",
    alpha = 0.2
  )
  ...
  accplot = accplot + geom_point(
    data = post_var,
    aes(x = condition, y = acc, group = src),
    color = "black",
    alpha = .2
  )
  
  accplot = accplot + labs(x = "Emphasis Instruction", y = "Mean Accuracy (%)", colour =
                             "Model")
