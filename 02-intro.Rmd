...
In this section, we are going to compare the three threshold model and the single threshold model for the forstmann (2008) data to evaluate which model is the most representative of the data. We begin by using a graphical method and compare plots.
 
### Assessing Descriptive Adequacy Graphically

We can <i>"assess descriptive adequacy"</i> graphically by taking a look at the plots above to see which of our models best fits the data. For ease we've plotted them on the same plot below. 
...
ModelCompPlots
```
The graphs show that values in the three threshold model (red circles) are more descriptively accurate than the single threshold model (blue circles).
