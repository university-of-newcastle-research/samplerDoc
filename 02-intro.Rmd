...
This may only be useful for some users, as what you do with your data is domain specific.

```{r plotppdata, include=TRUE, warning=FALSE, message=FALSE}
# Subject x condition Q1, median and Q3 respone time
pq <- data %>% 
...
# Mean accuracy for subject x condition 
acc <- data %>%
  group_by(condition, subject) %>%
  summarise(acc = mean(resp == stim))

# Dataframe with Q1, median, Q3 for response times and accuracy
pq <- cbind(pq, acc = acc$acc)
...
pp_pq <- pp_data_3b %>%
  ...
  summarise_at(vars(rt),
               list(Q1 = ~ quantile(., probs = 0.25),
                    median = median, 
                    Q3 = ~ quantile(., probs = 0.75)))
# Mean accuracy for subject x condition for posterior predictive data
pp_acc <- pp_data_3b %>%
  group_by(condition, pp_subj, subject) %>%
  summarise(acc = mean(resp == stim))

# Posterior predictive data with Q1, median, Q3 for response time and accuracy 
pp_pq <- cbind(pp_pq, acc = pp_acc$acc)

# Combine data with posterior predictive data 
...
# Make source column a factor and add labels
...
# Convert rt to milliseconds and acc percentage
