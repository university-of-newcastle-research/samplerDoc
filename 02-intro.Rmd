...
```{r}
save(sampled, file = "forst3b_sampled.RData")
```

## Simulating Posterior Predictive Data
...
```{r hideppdataplots, include=FALSE}
...
Now let's run through an example where we change our model slightly and assume that participants's level of caution does not change with emphasis instructions i.e. their threshold is identical in the `Accuracy`, `Neutral`, and `Speed` conditions. We will call this our "single threshold LBA model". The single threhold LBA model's log-likelihood has one `b` parameter, instead of three (see line 18, 31 and 64 below).
...
The checking procedure below is the same as the three threshold LBA model, but we'll use the single threshold LBA model's `sampled` object/data. 

```{r part10.5.1, eval=FALSE}
load("forstmann1b_sampled.RData")
```

As for the t generate 20 posterior predictive data samples for each model. 
```{r part10.5.3, eval=FALSE}
pp.data.1b <- generate.ppdata(sampled, n = 20)
```
